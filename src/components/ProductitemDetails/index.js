...
class ProductItemDetails extends Component {
  ...
  renderLoadingView = () => (
    <div className="products-details-loader-container" data-testid="loader">
      <Loader type="ThreeDots" color="#0b69ff" height="56" width="50" />
    </div>
  )
  ...
  onClickAddToCart = () => {
    ...
    const {id, title, brand, imageUrl, price} = productData

    const cartItems = JSON.parse(localStorage.getItem('cartData')) || []
    const existingItem = cartItems.find(eachItem => eachItem.id === id)

    let updatedCartItems = []
    if (existingItem) {
      updatedCartItems = cartItems.map(eachItem =>
        eachItem.id === id
      ...
      updatedCartItems = [
        ...cartItems,
        {id, title, brand, imageUrl, price, quantity},
      ]
    }
